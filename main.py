...
def main():
   ...
   #model.fit(train_images)
    ...
    if useUI:
        ...
        postAnalysis(model, userImgs)
        print(model.evaluate_generator(generator=validation_generator, steps=1000, workers=4))


def postAnalysis(model, userImgs):
    trial_images = []
    for i in userImgs:
        cv2.imshow("Make Sure These All Look Right", i)
        cv2.waitKey(0)
        i = np.transpose(i, (2, 0, 1))
        trial_images.append(i)
    #trial_generator = ImageDataGenerator(rescale=1./255).flow(np.array(trial_images))
    #predictions = model.predict_generator(trial_generator)
    trial_images = np.asarray(trial_images)
    #trial_images = np.expand_dims(trial_images, axis=0)
    predictions = model.predict_on_batch(trial_images)
    ...
    model.fit(trial_images, np.array(trial_labels), epochs=5)
